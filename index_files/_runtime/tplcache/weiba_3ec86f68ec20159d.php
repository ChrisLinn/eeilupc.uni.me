...
<div id="page-wrap">
  ...
  <div class="right-wrap">
     <div class="weiba-pic weiba-box1 clearfix">
       <div class="weiba-pic-title"><a href="<?php echo U('weiba/Index/weibaList');?>" class="right">更多微吧</a><h3>微吧推荐</h3></div>
       <ul>
          <?php if(is_array($weiba_recommend)): ?><?php $i = 0;?><?php $__LIST__ = $weiba_recommend?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li <?php if(is_int($i/5)){ ?>class="no-margin"<?php } ?>>
                <div class="weiba-pic-l"><a href="<?php echo U('weiba/Index/detail',array('weiba_id'=>$vo['weiba_id']));?>"><img src="<?php echo ($vo["logo"]); ?>" width="50" height="50"/></div><a href="<?php echo U('weiba/Index/detail',array('weiba_id'=>$vo['weiba_id']));?>" class="weiba-name"><?php echo ($vo["weiba_name"]); ?></a>
            </li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
       </ul>
     </div>  
     <div class="weiba-rank-list weiba-box1">
       <div class="weiba-pic-title"><h3>微吧排行榜</h3></div>
       <ul>
          <?php if(is_array($weiba_order)): ?><?php $i = 0;?><?php $__LIST__ = $weiba_order?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$v1): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li><i class="ico-num ico-num<?php echo ($i); ?>"></i>
                  <dl>
                     <dt><a href="<?php echo U('weiba/Index/detail',array('weiba_id'=>$v1['weiba_id']));?>"><img src="<?php echo ($v1["logo"]); ?>" width="30" height="30" /></a></dt>
                     <dd><p><a href="<?php echo U('weiba/Index/detail',array('weiba_id'=>$v1['weiba_id']));?>"><?php echo ($v1["weiba_name"]); ?></a></p><p>粉丝数：<?php echo (($v1["follower_count"])?($v1["follower_count"]):0); ?>  帖子数：<?php echo (($v1["thread_count"])?($v1["thread_count"]):0); ?></p></dd>
                  </dl>
              </li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
       </ul>
     </div>
    ...
    <?php echo Addons::hook('show_ad_space', array('place'=>'weiba_right'));?>
                 ...
                 <div id="col5" class="st-index-main">
                    <div class="extend-foot">
                      <!-- 微吧首页中部广告 -->
                      <?php echo Addons::hook('show_ad_space', array('place'=>'weiba_middle'));?>
                        <div class="weiba-box clearfix">
                           <div class="weiba-pic-title"><a class="right" href="<?php echo U('weiba/Index/postList');?>">更多帖子</a><h3>热帖推荐</h3></div>
                           <div class="hot-posts-list clearfix">
                              <ul>
                                <?php if(is_array($post_recommend)): ?><?php $i = 0;?><?php $__LIST__ = $post_recommend?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li <?php if(is_integer($i/2)){ ?>class="no-margin"<?php } ?>>
                                    <a class="right f9 f12" href="<?php echo U('weiba/Index/detail',array('weiba_id'=>$vo['weiba_id']));?>">[<?php echo (getShort($vo["weiba"],5)); ?>]</a><a class="tit" target="_blank" href="<?php echo U('weiba/Index/postDetail',array('post_id'=>$vo['post_id']));?>"><?php echo (getShort($vo["title"],14)); ?><?php if(strstr($vo['content'],'<img')){ ?><i class="ico-img"></i><?php } ?></a>
                                  </li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
                              </ul>
                           </div>
                        </div>
                        ...
                        <div class="weiba-box">
                          <div class="weiba-pic-title"><h3>帖子列表</h3></div>
                          <div class="my-weiba-list">
                             <ul>
                               <?php if(is_array($postList["data"])): ?><?php $i = 0;?><?php $__LIST__ = $postList["data"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li><div class="name"><span class="right num"><i class="arrow-weiba"></i><?php echo ($vo["reply_count"]); ?></span><a target="_blank" href="<?php echo U('weiba/Index/postDetail',array('post_id'=>$vo['post_id']));?>">【<?php echo ($vo["weiba"]); ?>】<?php echo ($vo["title"]); ?><?php if(strstr($vo['content'],'<img')){ ?><i class="ico-img"></i><?php } ?></a></div>
                                 <div class="f9"><span class="right"><?php echo ($user_info[$vo['last_reply_uid']]['space_link']); ?>&nbsp;&nbsp;<?php echo (friendlyDate($vo["last_reply_time"])); ?></span><span><?php if($vo['top']==2){ ?><i class="ico-bar-top mr5">置顶</i><?php } ?><?php if($vo['digest']==1){ ?><i class="ico-bar-fine mr5">精华</i><?php } ?><?php echo ($user_info[$vo['post_uid']]['space_link']); ?>&nbsp;&nbsp;<?php echo (friendlyDate($vo["post_time"])); ?>&nbsp;&nbsp;发布</span></div></li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
                             </ul>
                           </div>
                           <div id="page" class="page" style=""><?php echo ($postList["html"]); ?></div>
                        </div>
