...
<?php } ?>
<link href="__APP__/medal.css" rel="stylesheet" type="text/css" />
<script>
function showdetail(id,type){
	ui.box.load("<?php echo U('public/Medal/showdetail');?>",'勋章详情',null,{id:id,type:type});
}
</script>
<div id="page-wrap">
	   <div id="main-wrap">
          <div class="boxShadow">
	   	   	 <div class="find-type clearfix">
                       ...
                       <a href="<?php echo U('public/Task/index');?>" class="right">去做任务</a>
                       <h4 class="left"><img src="__APP__/image/ico-app-medal.png" />勋章馆</h4>
                       <div class="app-tab-menu" style="margin:0 0 0 110px;">
                           <ul>
                             <li><a href="<?php echo U('public/Medal/index' , array('type'=>2,'uid'=>$uid));?>" <?php if($type==2){ ?>class="current"<?php } ?>>全部勋章<?php if(($type)  ==  "2"): ?><span class="triangle"></span><?php endif; ?></a><i class="line"></i></li>
                             <li><a href="<?php echo U('public/Medal/index' , array('type'=>1,'uid'=>$uid));?>" <?php if($type==1){ ?>class="current"<?php } ?>><?php if($isme){ ?>我<?php }else{ ?>TA<?php } ?>的勋章<?php if(($type)  ==  "1"): ?><span class="triangle"></span><?php endif; ?></a></li>
                           </ul>
                       </div>
                   </div>
            </div>
                ...
                <?php if($type==1){ ?>
                <div class="medal-user clearfix">
                    <dl>
                        <dt><a href="<?php echo ($spaceurl); ?>"><img src="<?php echo ($face); ?>" width="50" height="50"/></a></dt>
                        <dd><p class="name"><a href="<?php echo ($spaceurl); ?>"><?php echo ($uname); ?></a>的勋章</p><p><?php if($isme){ ?>我<?php }else{ ?>TA<?php } ?>已获得<?php echo ($count); ?>枚勋章</p></dd>
                    </dl>
                </div>
                <?php } ?>
                 <div class="medal-content">
                    <div class="medal-title clearfix">
                        <h3>全部勋章<span class="f9">（共<?php echo ($count); ?>枚）</span></h3>
                    </div>
                    <div class="medal-list clearfix">
                        <ul>
                        <?php if(is_array($data)): ?><?php $i = 0;?><?php $__LIST__ = $data?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$v): ?><?php ++$i;?><?php $mod = ($i % 2 )?><li><a href="javascript:showdetail(<?php echo ($v["id"]); ?>,<?php echo ($type); ?>);" class="pic"><img src="<?php echo ($v["src"]); ?>" width="100" height="100"></a><a href="javascript:;" class="name"><?php echo ($v["name"]); ?></a></li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
                        </ul>
                        <a href="<?php if($showlast){ ?><?php echo U('public/Medal/index','uid='.$uid.'&type='.$type.'&p='.$lpage);?><?php }else{ ?>#<?php } ?>"><i class="<?php if($showlast){ ?>arrow-medal-l<?php }else{ ?>arrow-l-no<?php } ?>"></i></a>
                        <a href="<?php if($shownext){ ?><?php echo U('public/Medal/index','uid='.$uid.'&type='.$type.'&p='.$npage);?><?php }else{ ?>#<?php } ?>"><i class="<?php if($shownext){ ?>arrow-medal-r<?php }else{ ?>arrow-r-no<?php } ?>"></i></a>
                    </div>
                    <div class="page">
                       <?php echo ($html); ?>
                    </div>
