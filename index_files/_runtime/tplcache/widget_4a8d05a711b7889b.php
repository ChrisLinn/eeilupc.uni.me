...
<?php if(is_array($data)): ?><?php $i = 0;?><?php $__LIST__ = $data?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vl): ?><?php ++$i;?><?php $mod = ($i % 2 )?><?php $cancomment_old = empty($vl['app_row_id'])  ? 0 : 1; ?>
	...
	<dd class="content">
    <span event-node="show_admin" event-args="feed_id=<?php echo ($vl['feed_id']); ?>&uid=<?php echo ($vl['user_info']['uid']); ?>&feed_del=<?php echo CheckPermission('core_admin','feed_del');?>&channel_recommend=<?php echo CheckPermission('channel_admin','channel_recommend');?>&vtask_recommend=<?php echo CheckPermission('vtask_admin','vtask_recommend');?>" href="javascript:;" class="right f12 f9 hover" style="display:none;cursor:pointer">管理</span>
	...
	<?php if(($vl['user_info']['uid'] == $GLOBALS['ts']['mid'] && CheckPermission('core_normal','feed_del')) || CheckPermission('core_admin','feed_del')){ ?>
