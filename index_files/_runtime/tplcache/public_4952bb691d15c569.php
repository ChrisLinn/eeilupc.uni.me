	   ...
	   <div id="main-wrap">
          <div class="boxShadow">
	   	   	 <div class="find-type clearfix">
                       ...
                       <a href="<?php echo U('public/Medal/index');?>" class="right">查看我的勋章</a>
                       <h4 class="left"><img src="__APP__/image/ico-app-task.png" />任务</h4>
                       <div class="app-tab-menu" style="margin:0 0 0 110px;">
                           <ul>
                             <li><a href="<?php echo U('public/Task/index' , array('type'=>2));?>" <?php if($task_type==2){ ?>class="current"<?php } ?>>主线任务<?php if(($task_type)  ==  "2"): ?><span class="triangle"></span><?php endif; ?></a><i class="line"></i></li>
                             <li><a href="<?php echo U('public/Task/index' , array('type'=>1));?>" <?php if($task_type==1){ ?>class="current"<?php } ?>>每日任务<?php if(($task_type)  ==  "1"): ?><span class="triangle"></span><?php endif; ?></a><i class="line"></i></li>
                             <li><a href="<?php echo U('public/Task/customIndex');?>" <?php if($task_type==3){ ?>class="current"<?php } ?>>副本任务<?php if(($task_type)  ==  "3"): ?><span class="triangle"></span><?php endif; ?></a></li>
                           </ul>
                       </div>
                   </div>
            </div>
                ...
                <div class="task-stage">
                   	<div class="stage<?php echo ($task_level); ?>"><span class="a">新手任务</span><span class="b">进阶任务</span><span class="c">达人任务</span><span class="d">高手任务</span><span class="e">终极任务</span></div>
                </div>
                <?php } ?>
                 <div class="task-content">
                    ...
                    <?php if(is_array($list)): ?><?php $i = 0;?><?php $__LIST__ = $list?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$v): ?><?php ++$i;?><?php $mod = ($i % 2 )?><div class="task-list clearfix">

                            <ul>
                               <!--<li class="c0"><?php echo ($key+1); ?>.</li>-->
                               <li class="c1"><h4 class="strong"><?php echo ($v["step_name"]); ?> </h4><p><?php echo ($v["step_desc"]); ?> </p></li>
                               <li class="c2"><span>经验值<em>+<?php echo $v['reward']->exp; ?></em>点</span><span>财富值<em>+<?php echo $v['reward']->score; ?></em>点</span></li>
                               <li class="c3"><span><?php if ( $v['reward']->medal->name ){ ?><img src="<?php echo getImageUrl( $v['reward']->medal->src); ?>" width="50" height="50"/><?php echo $v['reward']->medal->name;}else{echo '<em>无</em>';} ?></span></li>
                               <li class="c4">
                               		<?php if($v['status']){ ?>
                               			<span id="step_<?php echo ($v["id"]); ?>">
                               			<?php if( $v['receive']){ ?>
											<img src="__APP__/image/ico-perform.png" />
										<?php }else{ ?>
											<a href="javascript:void(0);" onclick="complete_step(<?php echo ($v['id']); ?>,this,<?php echo ($task_type); ?>,<?php echo ($task_level); ?>)" class="btn-att-green">领取奖励</a>
										<?php } ?>
										</span>
                               		<?php }else{ ?>
	                               		<?php if($v['action']){ ?>
	                               		<a href="<?php echo ($v['action']); ?>" class="btn-cancel" target="_blank"><span>去做任务</span></a>
	                               		<?php }else{ ?>
	                               		<span>未完成</span>
	                               		<?php } ?>
                               		<?php } ?>
					...
					<?php if($iscomplete && $task_type==2){ ?>
						<?php if($receive){ ?>
						已完成<?php echo ($task_name); ?>
						<?php }else{ ?>
						<a href="javascript:void(0)" onclick="complete_task(<?php echo ($task_type); ?>,<?php echo ($task_level); ?>)" class="btn-cancel mt10"><span>已完成<?php echo ($task_name); ?>，领取奖励</span></a>
						<?php } ?>
					<?php } ?>
					</span>
                    </div>
