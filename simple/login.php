		...
		<div id="primary">
				...
				<article id="post-0" class="post no-results not-found">
					...
					<div class="entry-content" >
						...
						$password =$_POST["password"];// some code 
						if(con2db()){ $exec = "SELECT * FROM user WHERE Id='$id' ";      //sql语句，查询用户名称！ 
						$result = mysql_query($exec); 
						$rows = mysql_num_rows($result); 
						if($rows<=0) echo "用户不存在！"; 
						else { $exec = "SELECT * FROM user WHERE Id='$id' AND password='$password'"; 
						$result = mysql_query($exec); 
						$rows = mysql_num_rows($result);
						if($rows<=0) echo "密码错误！"; 
						else{ setcookie("id", "$id", time()+3600); 
						Header("Location:index.php");  } }}
						?>
						</br>
						</br>
						</br>
						<div style="position: relative;right: 400px"> 	<a  href="/index.php">返回首页</a></div>	
